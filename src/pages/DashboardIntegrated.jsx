    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-cyan-300/70">Please connect your wallet to view your dashboard</p>
        </div>
      </div>
    );
  }

  if (overviewLoading || portfoliosLoading || totalsLoading || walletLoading) {
    return (
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 px-3 sm:px-4 py-2 cyber-glass border border-neon-green/30 rounded-lg flex-shrink-0 w-fit">
          ...
          <span className="text-xs sm:text-sm font-medium text-neon-green uppercase tracking-wide">{status}</span>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <Link to="/dashboard/portfolio" className="cyber-glass border border-cyan-500/30 hover:border-cyan-500/80 rounded-xl p-4 sm:p-5 text-white transition-all group relative overflow-hidden">
        ...
        <Link to="/dashboard/team" className="cyber-glass border border-neon-orange/30 hover:border-neon-orange/80 rounded-xl p-4 sm:p-5 text-white transition-all group relative overflow-hidden">
          ...
          <p className="text-xl sm:text-2xl font-bold mb-2 text-neon-orange relative z-10">{directCount.toString()} Direct</p>
        ...
        <Link to="/dashboard/safe-wallet" className="cyber-glass border border-cyan-400/30 hover:border-cyan-400/80 rounded-xl p-4 sm:p-5 text-white transition-all group relative overflow-hidden">
          ...
          <NumberPopup
            value={oceanContractService.formatRAMA(totalSafeWalletRama)}
            label="Safe Wallet"
            className="text-xl sm:text-2xl font-bold mb-2 text-cyan-400 relative z-10"
          />
          <div className="flex items-center gap-1 text-xs text-cyan-300/90 relative z-10">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-cyan-300/90 mt-1">
                  {oceanContractService.formatRAMA(activePortfolio?.creditedRama || '0')} / {maxCap.toFixed(2)} RAMA
                  <span className="ml-1 text-neon-green">{isBooster ? '(250% Cap)' : '(200% Cap)'}</span>
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-4">
                ...
                <div className="p-3 sm:p-4 cyber-glass rounded-xl border border-neon-green/30 hover:border-neon-green/80 transition-all group">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-neon-green group-hover:text-neon-glow transition-all">{directCount.toString()}</p>
                </div>
                <div className="p-3 sm:p-4 cyber-glass rounded-xl border border-neon-orange/30 hover:border-neon-orange/80 transition-all group">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-neon-orange group-hover:text-neon-glow transition-all truncate">
                    {currentSlabName}
                  </p>
                  <p className="text-xs text-neon-orange/70 mt-0.5">Level {slabIndex.toString()}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 p-2 rounded-lg hover:bg-cyan-500/5 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-neon-green">Earning {dailyRate}% daily</p>
                  <p className="text-xs text-cyan-400/50">Active now</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-2 rounded-lg hover:bg-cyan-500/5 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-cyan-300">Team Network</p>
                  <p className="text-xs text-cyan-300">{directCount.toString()} direct members</p>
                  <p className="text-xs text-cyan-400/50">View details</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-2 rounded-lg hover:bg-cyan-500/5 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-cyan-300">Slab Level</p>
                  <p className="text-xs text-neon-orange">Level {slabIndex.toString()} Active</p>
                  <p className="text-xs text-cyan-400/50">{currentSlabName}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <Link to="/dashboard/analytics" className="cyber-glass rounded-xl p-5 border border-cyan-500/30 hover:border-cyan-500/80 transition-all group relative overflow-hidden">
          ...
          <p className="text-2xl font-bold text-neon-green relative z-10">{portfolioCapProgress.toFixed(0)}% Progress</p>
          <p className="text-xs text-cyan-300/90 mt-1 relative z-10">Portfolio growth</p>
        </Link>
        ...
        <div className="cyber-glass rounded-xl p-5 border border-cyan-500/30 hover:border-cyan-500/80 relative overflow-hidden transition-all">
          ...
          <p className="text-2xl font-bold text-cyan-300">Level {royaltyTier.toString()}</p>
          <p className="text-xs text-cyan-300/90 mt-1">Lifetime Eligible</p>
        </div>
      </div>
    </div>
  );
